...
(async () => {
    try {
        const { data, error } = await supabase.from('tickets').select('*, assets(name), profiles!created_by(name)').limit(1);
        console.log("Full Query Result:", data, "Error:", JSON.stringify(error, null, 2));
    } catch (e) {
        console.error(e);
    }
})();
