...
const HRPayroll: React.FC<HRPayrollProps> = ({ user, selectedProjectId }) => {
    ...
    const downloadExcel = () => {
        const headers = ['الموظف', 'الرقم القومي', 'الرقم الوظيفي', 'القسم', 'نوع الحساب', 'رقم الحساب', 'الأساسي', 'نسبة الإنجاز', 'المتغير الفعلي', 'إجمالي الراتب', 'ساعات إضافي', 'قيمة الإضافي', 'أيام غياب', 'أيام جزاء', 'قيمة الخصم', 'تأمين', 'ضرائب', 'الشهداء', 'الصافي', 'الحالة'];
        const data = payrollData.map(r => [
            r.employee_name,
            r.national_id || '-',
            r.employee_code || '-',
            r.employee_department || '-',
            r.transfer_account_type || '-',
            r.transfer_account_number || '-',
            r.basic_salary,
                ...
                if (targetRecIdx !== -1) {
                    ...
                    if (rec.status !== 'finalized') {
                        ...
                        if (compPercent !== undefined && !isNaN(compPercent)) {
                            ...
                            if (newRec.target_volume && newRec.target_volume > 0) {
                                newRec.target_achieved = Math.round((compPercent / 100) * newRec.target_volume);
                            } else {
                                newRec.target_achieved = Math.round(compPercent);
                            }
                        }
